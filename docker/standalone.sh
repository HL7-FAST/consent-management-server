...
# AEGIS 2025 - Custom conf execution for Docker
# Read an optional environment variable configuration file.
# This would be written from the calling start.sh within the
# Docker container.
if [ "x$ENV_CONF" = "x" ]; then
    ENV_CONF="$DIRNAME/env.conf"
fi
if [ -r "$ENV_CONF" ]; then
    . "$ENV_CONF"
fi
